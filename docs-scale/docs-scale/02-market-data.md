...
- 틱을 받을 때마다 **스트림 생존 시각**(`market-data:last_tick_at`)을 따로 갱신한다.
- `get_cached_price()`는 **스트림 생존 시각**이 `PRICE_MAX_AGE_SECONDS`를 넘었을 때 **`None`을 반환한다** — 호출부 입장에서는 "아직 시세 없음"과 같아서 기존 분기가 그대로 동작한다.
- 자금이 움직이는 경로(시장가 주문 접수, 워커 청산 판정, 매칭)는 `None`이면 **수행하지 않는다.** 화면 표시 경로는 마지막 값을 회색으로 보여주는 등 완화해도 된다.

> **판정 기준은 코인별 나이가 아니라 스트림 생존이다.** 초안은 "`received_at`이 오래된 값을 만나면 None"이었는데, 구현하면서 이게 틀렸다는 것이 드러났다. Upbit ticker는 **체결이 있을 때만** 프레임을 보내므로 거래가 뜸한 코인은 스트림이 멀쩡해도 몇 분씩 갱신이 없다. 코인별 나이로 판정하면 그런 코인은 **정상 상황에서 주문이 거부되고 손절 판정이 스킵된다** — 바로 아래 열린 질문이 "정상 상황에서 거부되면 안 된다"고 못 박은 그 조건을 스스로 위반한다. 그래서 나이는 스트림 단위로 재고, 코인별 `received_at`은 관측용([06](06-observability.md) 3.2절)으로만 남긴다.

> **열린 질문**: `PRICE_MAX_AGE_SECONDS`를 얼마로 할 것인가. 워커 tick이 10초이므로 그보다는 커야 하고(정상 상황에서 거부되면 안 됨), 손절이 늦으면 안 되므로 너무 크면 안 된다. 30초를 초안으로 두되 [06](06-observability.md)의 실측 후 확정한다. **스트림 단위 판정으로 바뀌면서 이 값은 "가장 한가한 코인의 체결 간격"이 아니라 "틱이 하나도 안 들어오면 죽은 것으로 볼 시간"이 됐다** — 전 종목 합산 틱은 한가한 시간대에도 초 단위로 들어오므로 30초는 넉넉하다.
