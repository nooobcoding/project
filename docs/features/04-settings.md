...
| 비밀번호 변경 폼 | Input×3 (현재/새/확인) | 새 비밀번호는 회원가입과 동일 조건(8자+영문+숫자)이며, 현재 비밀번호와 같을 수 없다 |
...
| 위치 | 이벤트 | 처리 | 결과 |
|---|---|---|---|
| 비밀번호 변경 저장 | Click | 현재 비밀번호 검증 → 신규 비밀번호가 현재 비밀번호와 다른지 확인 → 변경 API 호출 | 성공 토스트 / 실패: "현재 비밀번호가 올바르지 않습니다" 또는 "새 비밀번호가 현재 비밀번호와 같습니다" |
